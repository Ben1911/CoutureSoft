    ...
    public partial class FrmConfiguration : Form
    {
        ...
        private void confirmer_TextChanged(object sender, EventArgs e)
        {
            if (txtpassword.Text.Equals(confirmer.Text))
            {
                ...
                {
                    ImplementeConnexion.Instance.Initialise();
                    ClsConfiguration.SelectionnerBd(txtdatabase);
                }
                else { }
            }
            else
        ...
        private void Table_Click_1(object sender, EventArgs e)
        {
            try
            {
                btnSave.Text = "Modifier";
                id = Table.SelectedCells[0].Value.ToString();
                Agent.Text = Table.SelectedCells[1].Value.ToString();
                username.Text = Table.SelectedCells[2].Value.ToString();
                pwd.Text = Table.SelectedCells[3].Value.ToString();
                niveau.Text = Table.SelectedCells[4].Value.ToString();
                conf.Text = pwd.Text;
                a.IdAgent = pro.IdCombo("liste_agent", Agent.Text, "Noms");
                d.RetourPhoto("SQLServer", a.IdAgent, profil);
            }
            catch (Exception ex)
            { MessageBox.Show(ex.Message); }
        }
        ...
        private void rech_TextChanged(object sender, EventArgs e)
        {
            try
            {
                Table.DataSource = d.recherche_UsignSql("select * from Liste_User where id is not null and  Username LIKE '%" + rech.Text + "%'");
